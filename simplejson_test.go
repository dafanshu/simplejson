...
func TestPathWillOverwriteExisting(t *testing.T) {
	...
	js1, err1 := NewJson([]byte(`{"this":{"a":0.1,"b":"bb","c":"cc"}}`))
	js.Set("js1", js1)
	assert.Equal(t, nil, err1)
	data, err2 := js.MarshalJSON()
...
func TestJson_Bytes(t *testing.T) {
	data := simplejson.New()
	data.Set("Error", "testtestsestst")
	b, err := data.Get("Error").Bytes()
	assert.Equal(t, nil, err)
	fmt.Println(b)
}

func TestSout(t *testing.T) {
	fix := "{\"meta\":{\"rowCount\":2,\"sql\":\"SELECT * FROM log_his limit 0,2\"},\"refId\":\"A\",\"tables\":[{\"columns\":[{\"text\":\"log_id\"},{\"text\":\"log_type\"},{\"text\":\"entity_id\"},{\"text\":\"user_id\"},{\"text\":\"account\"},{\"text\":\"username\"},{\"text\":\"op_type\"},{\"text\":\"content\"},{\"text\":\"dev_content\"},{\"text\":\"module\"},{\"text\":\"bread_name\"},{\"text\":\"application_id\"},{\"text\":\"ip\"},{\"text\":\"browser\"},{\"text\":\"operate_sys\"},{\"text\":\"operate_time\"}],\"rows\":[[1000001,1,1999,1,\"admin\",\"HD Admin\",8,\"登录\",\"\",\"登录\",\"首页\",6,\"127.0.0.1\",\"Chrome\",\"Windows 7\",\"2019-02-20T18:26:32Z\"],[1000002,2,1999,1,\"admin\",\"HD Admin\",2,\"\",\"\",\"电量申报设置\",\"零售市场管理/\\n电量申报管理\",6,\"127.0.0.1\",\"Chrome\",\"Windows 7\",\"2019-02-20T18:28:09Z\"]]}]}"
	js1, err1 := NewJson([]byte(fix))
	assert.Equal(t, nil, err1)

	_, ok1 := js1.CheckGet("meta")
	fmt.Println("ok1>>>", ok1)
...
func TestString(t *testing.T) {
	simplejson.NewJson(nil)
	js := simplejson.New()
	js.Set("test", "test")
	js1, _ := NewJson([]byte("{\"this\":{\"a\":0.1,\"b\":\"bb\",\"c\":\"cc\"}}"))
	js.Set("js1", js1)
	ds, ok := js.CheckGet("js1")
	if ok {
		aa, _ := ds.MarshalJSON()
